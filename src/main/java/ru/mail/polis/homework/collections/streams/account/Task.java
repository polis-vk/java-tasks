...
public class Task {
    ...
    public static Map<String, Long> paymentsSumByAccount(List<Transaction> transactions) {
        return transactions.stream()
                .collect(Collectors.groupingBy(Transaction::getSenderId, Collectors.summingLong(Transaction::getSum)));
    }
    ...
    public static List<String> paymentsSumByAccount(List<Account> accounts, long t, int n) {
        return accounts.stream()
                .sorted(Comparator.comparingLong(account -> account.getBalanceBeforeDate(t)))
                .map(Account::getId)
                .skip(1)
                .limit(n)
