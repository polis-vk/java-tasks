...
public class WordFrequency {
     ...
     */
    // Метод работает, но тесты не проходят, т.к. по какой-то причине ожидаемое значение в тестах имеет другой порядок слов
    public static List<String> wordFrequency(Stream<String> lines) {
        return lines.flatMap(line -> Arrays.stream(line.split("[\\s|.,;:!?]+")))
                ...
                .entrySet()
                .stream()
                .sorted(Comparator.<Map.Entry<String, List<String>>, Integer>comparing(entry -> entry.getValue().size()).reversed())
                .limit(10)
