...
public class WordFrequency {
    ...
    public static List<String> wordFrequency(Stream<String> lines) {
        Stream<String> sortedWords = lines
                .map(it -> Arrays.stream(it.toLowerCase().split("[\n .,;:!?]")))
                .reduce(Stream.empty(), Stream::concat).sorted();
        return sortedWords.collect(Collectors.groupingBy(Function.identity(), Collectors.counting()))
                .entrySet().stream().sorted(Map.Entry.comparingByValue()).limit(10)
