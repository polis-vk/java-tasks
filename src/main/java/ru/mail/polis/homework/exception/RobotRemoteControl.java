...
public class RobotRemoteControl {
    ...
    private RobotConnectionManager connectionManager = new RobotConnectionManagerImpl(new HashMap<>());
     ...
     */
    public void moveTo(int robotId, int toX, int toY) throws RobotIdException, RobotConnectionException {
        for (int i = 0; true; i++) {
            try (RobotConnection connection = connectionManager.getConnection(robotId)) {
                connection.moveRobotTo(toX, toY);
                return;
            } catch (RobotConnectionException | RobotIdException e) {
                if (i == 2) {
                    throw e;
                }
            }
