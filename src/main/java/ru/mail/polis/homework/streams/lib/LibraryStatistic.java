    ...
    public Map<User, Integer> specialistInGenre(Library library, Genre genre) {
        return library.getArchive().stream()
                ...
                .filter(archivedData -> archivedData.getReturned() != null
                        && TimeUnit.MILLISECONDS.toDays(archivedData.getReturned().getTime() - archivedData.getTake().getTime()) >= 14
                        || archivedData.getReturned() == null
                        && TimeUnit.MILLISECONDS.toDays(new Timestamp(System.currentTimeMillis()).getTime() - archivedData.getTake().getTime()) >= 14)
                .collect(Collectors.groupingBy(ArchivedData::getUser, Collectors.summingInt(o -> o.getBook().getPage())));
    }
    ...
    public List<User> unreliableUsers(Library library) {
        return library.getArchive().stream()
                ...
                .filter(userListEntry -> userListEntry.getValue().stream()
                        .filter(archivedData -> archivedData.getReturned() != null
                                && TimeUnit.MILLISECONDS.toDays(archivedData.getReturned().getTime() - archivedData.getTake().getTime()) > 30
                                || archivedData.getReturned() == null
                                && TimeUnit.MILLISECONDS.toDays(new Timestamp(System.currentTimeMillis()).getTime() - archivedData.getTake().getTime()) > 30)
                        .count() > userListEntry.getValue().size() * 0.5)
