...
public class DoubleAdvancedTask {
    ...
    public static String equation(int a, int b, int c, int d) {
        ...
        if (Math.pow(r, 2) < Math.pow(q, 3)) {
            ...
            roots[0] = -2 * Math.sqrt(q) * Math.cos(phi) - a1 / 3;
            roots[1] = -2 * Math.sqrt(q) * Math.cos(phi + (2 * Math.PI / 3)) - a1 / 3;
            roots[2] = -2 * Math.sqrt(q) * Math.cos(phi - (2 * Math.PI / 3)) - a1 / 3;
        } else {
            roots[0] = -2 * Math.cbrt(r) - a1 / 3;
