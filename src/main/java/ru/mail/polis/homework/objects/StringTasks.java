...
public class StringTasks {
    ...
    public static Number valueOf(String str) {
        ...
        int eCount = numberOfChar('e', str);
        int dotCount = numberOfChar('.', str);
        if (dotCount > 1 || eCount > 1 || numberOfChar('-', str) > 2) {
            return null;
        }
    ...
    private static int numberOfChar(char desiredCharacter, String str) {
        int count = 0;
        for (char symbol : str.toCharArray()) {
            if (symbol == desiredCharacter) {
                count++;
            }
        }
        return count;
    ...
    private static String toENotation(String str) {
        StringBuilder correctsCharacters = new StringBuilder();
        for (char character : str.toCharArray()) {
            if (Character.isDigit(character)
                    || character == '-'
                    || character == 'e'
                    || character == '.') {
                correctsCharacters.append(character);
            }
        }
        String eNotation = correctsCharacters.toString();
