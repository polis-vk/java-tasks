...
public class TextFilterManagerTest {
    ...
    public void analyzeOnlyLongSpaceAnalyzer() {
        TextFilterManager manager = new TextFilterManager(new TextAnalyzer[]{TextAnalyzer.createLongSpaceAnalyzer()});
        assertEquals("LONG_SPACE", manager.analyze("       я Петя :(").toString());
        assertEquals("LONG_SPACE", manager.analyze("   ").toString());
        assertEquals("GOOD", manager.analyze(null).toString());
        assertEquals("LONG_SPACE", manager.analyze("Это  мой лучший друг: всегда в хорошем настроении и не задаёт лишних вопросов.").toString());
