...
public class TextFilterManagerTest {
    ...
    public void mycustomTest() {
        TextFilterManager manager = new TextFilterManager(new TextAnalyzer[]{TextAnalyzer.createLevenshteinAnalyzer("Привет, я Петя :(")});
        assertEquals("NO_SIMILAR_STRING", manager.analyze("aaaaa").toString());
        assertEquals("NO_SIMILAR_STRING", manager.analyze("Привет, я ").toString());
        assertEquals("GOOD", manager.analyze("Привет, я Петя").toString());
        assertEquals("NO_SIMILAR_STRING", manager.analyze("Привет, я Петя вот мой cvv").toString());
        assertEquals("NO_SIMILAR_STRING", manager.analyze("Ооооооочень длиннннннаааааяяяя стрроооооооккккаааааа").toString());
    }
}