...
public class TextFilterManagerTest {
    ...
    @Test
    public void analyzeOnlyTooFewWordsFilter() {
        TextFilterManager manager = new TextFilterManager(new TextAnalyzer[]{TextAnalyzer.createTooFewWordsAnalyzer(4)});
        assertEquals("TOO_FEW_WORDS", manager.analyze("Привет, я Петя :(").toString());
        assertEquals("TOO_FEW_WORDS", manager.analyze("").toString());
        assertEquals("TOO_FEW_WORDS", manager.analyze(null).toString());
        assertEquals("GOOD", manager.analyze("Скажите код из смс  :(").toString());
        assertEquals("GOOD", manager.analyze("Скажите код из смс пожалуйста :|").toString());
        assertEquals("TOO_FEW_WORDS", manager.analyze("Ооооооочень длиннннннаааааяяяя стрроооооооккккаааааа").toString());
    }
    ...
    public void analyzeAllFiltersGood() {
        TextFilterManager manager = new TextFilterManager(new TextAnalyzer[]{
                ...
                TextAnalyzer.createTooFewWordsAnalyzer(0)});
        assertEquals("GOOD", manager.analyze("Привет, я Петя :-(").toString());
        assertEquals("GOOD", manager.analyze("").toString());
        assertEquals("GOOD", manager.analyze(null).toString());
    ...
    public void analyzeAllFiltersOne() {
        TextFilterManager manager = new TextFilterManager(new TextAnalyzer[]{
                ...
                TextAnalyzer.createSpamAnalyzer(new String[]{"пинкод", "смс", "cvv"}),
                TextAnalyzer.createTooLongAnalyzer(20),
                TextAnalyzer.createTooFewWordsAnalyzer(2)});
        assertEquals("NEGATIVE_TEXT", manager.analyze("Привет, я Петя :(").toString());
        assertEquals("TOO_LONG", manager.analyze("Скажите Код Из Смс :-(").toString());
        assertEquals("SPAM", manager.analyze("смс пожалуйста ;|").toString());
        assertEquals("TOO_FEW_WORDS", manager.analyze("пожалуйста :)").toString());
    }
    ...
    private void manyFilters(boolean withPriority) {
        TextFilterManager manager = new TextFilterManager(new TextAnalyzer[]{
                ...
                TextAnalyzer.createSpamAnalyzer(new String[]{"пинкод", "смс", "cvv"}),
                TextAnalyzer.createTooLongAnalyzer(20),
                TextAnalyzer.createTooFewWordsAnalyzer(3)});
        if (withPriority) {
            ...
            assertTrue(Arrays.asList("NEGATIVE_TEXT", "TOO_LONG", "SPAM", "TOO_FEW_WORDS").contains(
