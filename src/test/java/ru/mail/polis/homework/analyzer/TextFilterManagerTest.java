...
public class TextFilterManagerTest {
    ...
    public void analyzeCustomFilter() {
        TextFilterManager manager = new TextFilterManager(new TextAnalyzer[]{
                ...
                TextAnalyzer.createSpamAnalyzer(new String[]{"пинкод", "смс", "cvv"}),
                TextAnalyzer.createTooLongAnalyzer(20),
                TextAnalyzer.createCustomAnalyzer(new String[]{"(", "!", "я"}, new int[]{3, 4 ,5})});
        assertEquals("CUSTOM", manager.analyze("Привет, яяяяяя").toString());
        assertEquals("NEGATIVE_TEXT", manager.analyze("((( не работает :(").toString());
        assertEquals("SPAM", manager.analyze("!!!!! смс !!!!").toString());
        assertEquals("CUSTOM", manager.analyze("Скажите (((((").toString());
