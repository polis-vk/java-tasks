...
public class TextFilterManagerTest {
    ...
    public void analyzeOnlyCustomFilter() {
        ...
        assertEquals("CUSTOM", manager.analyze("Ыранские цыгане украли кашалота").toString());
        assertEquals("GOOD", manager.analyze("Буква ъ хороша").toString());
        assertEquals("CUSTOM", manager.analyze("Не ожидал тут увидеть  ъуь").toString());
        assertEquals("GOOD", manager.analyze("").toString());
        assertEquals("GOOD", manager.analyze(null).toString());
    }
}